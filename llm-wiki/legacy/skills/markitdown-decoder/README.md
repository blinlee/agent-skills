# Archived llm-wiki Decoder Router

This directory is archived historical material. It is intentionally not a valid skill and must not be discovered or invoked as an active decoder. The active document conversion boundary is the installed `/anything2md` skill documented in the repository-root `SKILL.md`.

The notes below describe the old local decoder router for audit and migration context only.
