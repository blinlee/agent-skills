...
function buildDefaultWikiSchema(): string {
  return [
    ...
    '',
    '## 分层',
    '- `raw/inbox/`: 给人投递新来源的短期入口。intake scan 应尽快把条目移出这里，避免定时任务反复扫描历史 raw 文件。',
    '- `raw/objects/`: atlas 级原始材料的分片内容寻址对象库（`raw/objects/<sha-prefix>/<sha>/...`）。查待处理工作应使用 intake ledger，不要扫描这棵目录做决策。',
    '- `raw/staged/`、`raw/archive/`、`raw/rejected/`: 单 wiki 摄入时保存的不可变证据。受管 raw 文件在正文前带有 `source_ref`、`ingested`、`sha256` frontmatter。可以读取，但采集后不得编辑；修正应写入 `wiki/`、`review/` 或 `taxonomy/`。',
    ...
    '',
    '## 页面分区',
    '- `wiki/sources/`: 来源卡片和以 provenance 为核心的来源页。',
    '- `wiki/readings/`: Obsidian 可见的完整阅读镜像；它来自 raw/decoded 正文，用于人读和图谱导航，不替代 raw 证据。',
    ...
    '- `wiki/bridges/`: 经人工批准的跨 wiki 连接落地页，用于说明另一个 wiki 中材料与当前 wiki 的关系。',
    '',
    '## 约定',
    '- 使用 lowercase kebab-case slug。',
    '- 优先使用带分区的 wikilink：`[[sources/source-slug|Title]]`、`[[readings/source-slug|完整原文]]`、`[[entities/entity-slug|Title]]`、`[[concepts/concept-slug|Title]]`、`[[syntheses/synthesis-slug|Title]]`、`[[bridges/bridge-slug|Title]]`。',
    ...
    '',
    '## 页面阈值',
    '- 普通来源入库应直接生成可用的 source/readings/concept/entity 页面；稀疏、冲突或歧义材料才进入内部治理状态。',
    '- 不要为路过式提及创建结构治理决定；必要时用后续 govern 合并、重命名或重构页面。',
    '- 同一稳定主题应更新既有页面，不要重复建页。',
    '- 页面长到难以快速扫描时，应拆分或重构。',
    '- uncertain、conflicting、merge-prone 或边界性知识应进入内部治理状态，不要静默固化。',
    '- taxonomy/category/profile/bridge 等结构治理仍需明确批准；普通 source-derived concept/entity 页面不是后续 govern 的待完成入库步骤。',
    '',
    '## 人在回路分类',
