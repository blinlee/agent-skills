...
export type RefreshSemanticOverviewsResult = {
  writtenFiles: string[]
  overviewCount: number
  removedCount: number
}

type SemanticSection = 'entities' | 'concepts'
...
export async function refreshSemanticOverviews(input: {
...
}): Promise<RefreshSemanticOverviewsResult> {
  ...
  const desired = await resolveOverviewSlugCollisions(
    synthesesDirectory,
    buildOverviewPages(semanticPages, sourcePages),
  )
  ...
  const removedIndexEntries: string[] = []
  let removedCount = 0
...
async function listSemanticPages(root: string, section: SemanticSection): Promise<SemanticPage[]> {
...
async function listSourcePages(root: string): Promise<SourcePage[]> {
...
async function resolveOverviewSlugCollisions(synthesesDirectory: string, pages: OverviewPage[]): Promise<OverviewPage[]> {
  const reservedSlugs = new Set<string>()
  const resolvedPages: OverviewPage[] = []
...
async function chooseGeneratedOverviewSlug(
  synthesesDirectory: string,
  baseSlug: string,
...
function buildOverviewPages(pages: SemanticPage[], sourcePages: SourcePage[]): OverviewPage[] {
  const groups = new Map<string, SemanticPage[]>()
  for (const page of pages) {
  ...
  return [
    ...
    .filter((page): page is OverviewPage => page !== null)
    .sort((left, right) => left.slug.localeCompare(right.slug))
}

function buildWikiOverviewPage(sourcePages: SourcePage[], semanticPages: SemanticPage[]): OverviewPage | null {
...
function buildOverviewPage(group: SemanticPage[]): OverviewPage | null {
...
async function listGeneratedOverviewSlugs(synthesesDirectory: string): Promise<string[]> {
  let entries: DirectoryEntry[]
  try {
...
function isGeneratedOverviewMarkdown(markdown: string): boolean {
  return markdown.includes(`generatedBy: ${JSON.stringify(GENERATED_BY)}`)
}

async function refreshSourceOverviewLinks(root: string, overviews: OverviewPage[]): Promise<string[]> {
  ...
  const overviewEntriesBySource = new Map<string, string[]>()
...
function replaceManagedSection(markdown: string, heading: string, entries: string[]): string {
...
function formatOverviewMarkdown(page: OverviewPage): string {
...
function formatIndexEntry(page: Pick<OverviewPage, 'slug' | 'title'>): string {
  return `- [[syntheses/${page.slug}|${page.title}]]`
}

function extractSourceLinks(markdown: string): SourceLink[] {
...
function extractEvidenceLines(markdown: string): string[] {
