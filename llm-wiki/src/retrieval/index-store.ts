...
import { hashRawBody, readRawManifest, stripManagedRawFrontmatter, type RawManifest, type RawManifestEntry } from '../intake/raw-store.js'
import type { EntityConceptGraphIndex } from './entity-graph.js'
import type { RetrievalGraphIndex } from './graph.js'
import type { RetrievalTaxonomyIndex } from './taxonomy.js'
import type { ChunkIndexEntryV2, ChunkIndexStateV2, LexicalIndexState } from './types.js'

export type RetrievalIndexStore = {
  chunks: ChunkIndexEntryV2[]
  chunksById: Map<string, ChunkIndexEntryV2>
  lexical: LexicalIndexState
  graph: RetrievalGraphIndex | null
  entityGraph: EntityConceptGraphIndex | null
  taxonomy: RetrievalTaxonomyIndex | null
  readinessDiagnostics: string[]
}

export async function loadRetrievalIndex(knowledgeRoot: string): Promise<RetrievalIndexStore | null> {
  ...
  try {
    ...
    const chunkState = JSON.parse(chunksRaw) as Partial<ChunkIndexStateV2>
    const lexical = JSON.parse(lexicalRaw) as Partial<LexicalIndexState>
    ...
    return {
      ...
      lexical: lexical as LexicalIndexState,
...
async function rawBackedReadinessDiagnostics(root: string, chunks: ChunkIndexEntryV2[]): Promise<string[]> {
  const manifest = await readRawManifest(root)
  const diagnostics: string[] = []
  const checkedRawPaths = new Set<string>()
...
function findManifestEntry(manifest: RawManifest, sourceRef: string, relativeRawPath: string | null): RawManifestEntry | null {
...
function parseGraphIndex(raw: string | null): RetrievalGraphIndex | null {
  if (!raw) {
    return null
  }
  const parsed = JSON.parse(raw) as Partial<RetrievalGraphIndex> & { version?: number }
...
function parseEntityGraphIndex(raw: string | null): EntityConceptGraphIndex | null {
  if (!raw) return null
...
function parseTaxonomyIndex(topicNodesRaw: string | null, topicsRaw: string | null, aliasesRaw: string | null, redirectsRaw: string | null, categoryGraphRaw: string | null): RetrievalTaxonomyIndex | null {
...
function parseTopicNodes(raw: string | null): RetrievalTaxonomyIndex['topicNodes'] {
  if (!raw) {
    return []
  }
  const parsed = JSON.parse(raw) as { version?: number; schema?: string; topics?: unknown }
...
function parseTopics(raw: string | null): RetrievalTaxonomyIndex['topics'] {
  if (!raw) {
    return []
  }
...
function parseCategoryEdges(raw: string | null): RetrievalTaxonomyIndex['categoryEdges'] {
  if (!raw) {
    return []
  }
