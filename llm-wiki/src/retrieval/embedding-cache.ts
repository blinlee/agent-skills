...
import type { EmbeddingCacheLoadResult, EmbeddingCacheRecord, EmbeddingProviderConfig, EmbeddingProviderName } from './embedding-types.js'

export function embeddingCacheKey(input: { provider: EmbeddingProviderName; model: string; textSha256: string }): string {
  return `${input.provider}:${input.model}:${input.textSha256}`
}

export function embeddingCacheDirectory(knowledgeRoot: string, config: Pick<EmbeddingProviderConfig, 'provider' | 'model'>): string {
  return path.join(path.resolve(knowledgeRoot), 'system', 'index', 'embeddings', safeProviderModel(config.provider, config.model))
}

export function embeddingCachePath(knowledgeRoot: string, config: Pick<EmbeddingProviderConfig, 'provider' | 'model'>): string {
  return embeddingCacheDatabasePath(knowledgeRoot, config)
}

export function embeddingCacheDatabasePath(knowledgeRoot: string, config: Pick<EmbeddingProviderConfig, 'provider' | 'model'>): string {
  return path.join(embeddingCacheDirectory(knowledgeRoot, config), 'vectors.db')
}

export async function loadEmbeddingCache(filePath: string): Promise<EmbeddingCacheLoadResult> {
  return buildCacheLoadResult(readSqliteEmbeddingCache(filePath) ?? [])
}

export async function writeEmbeddingCache(filePath: string, records: EmbeddingCacheRecord[]): Promise<void> {
...
function buildCacheLoadResult(records: EmbeddingCacheRecord[]): EmbeddingCacheLoadResult {
  const recordsByCacheKey = new Map<string, EmbeddingCacheRecord>()
  const recordsByTextSha256 = new Map<string, EmbeddingCacheRecord>()
...
function readSqliteEmbeddingCache(dbPath: string): EmbeddingCacheRecord[] | null {
  ...
  try {
    ...
    const rows = db.prepare('SELECT * FROM vectors ORDER BY cache_key, chunk_id').all() as SqliteEmbeddingRow[]
...
function writeSqliteEmbeddingCache(dbPath: string, records: EmbeddingCacheRecord[]): void {
...
function ensureSqliteEmbeddingSchema(db: Database.Database): void {
  db.exec(`
...
function embeddingRecordToSqliteRow(record: EmbeddingCacheRecord): SqliteEmbeddingRow {
  return {
...
function embeddingRecordFromSqliteRow(row: SqliteEmbeddingRow): EmbeddingCacheRecord {
...
function safeProviderModel(provider: string, model: string): string {
...
function isEmbeddingCacheRecord(value: Partial<EmbeddingCacheRecord>): value is EmbeddingCacheRecord {
...
function isEmbeddingProviderName(value: unknown): value is EmbeddingProviderName {
