...
import type { EmbeddingCacheRecord, EmbeddingProviderConfig } from './embedding-types.js'
...
export type HybridScoreInput = {
  ...
  embeddingRecords: Map<string, EmbeddingCacheRecord>
  queryVector: number[] | null
  providerConfig: Pick<EmbeddingProviderConfig, 'provider' | 'model'> | null
  graphBoosts?: Map<string, { score: number; reasons: string[] }>
  taxonomyBoosts?: Map<string, { score: number; reasons: string[] }>
}

export type HybridScoreEntry = {
...
export function scoreHybrid(input: HybridScoreInput): HybridScoreEntry[] {
  const entries: HybridScoreEntry[] = []
...
function metadataScoreForChunk(chunk: ChunkIndexEntryV2, hasLexicalMatch: boolean): number {
...
function sourceCardBoilerplatePenalty(chunk: ChunkIndexEntryV2): number {
