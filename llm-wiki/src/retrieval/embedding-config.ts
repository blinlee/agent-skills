...
import type { EmbeddingProviderConfig, EmbeddingProviderName, EmbeddingRequestFormat } from './embedding-types.js'

export type EmbeddingConfigSource = 'config'

export type ResolvedEmbeddingProviderConfig = EmbeddingProviderConfig & {
  source: EmbeddingConfigSource
  configPath?: string
}

type HostLocalEmbeddingConfig = Partial<{
...
type HostLocalConfig = {
  embeddingProvider?: HostLocalEmbeddingConfig
  embedding?: HostLocalEmbeddingConfig
}

export function loadEmbeddingProviderConfig(env: NodeJS.ProcessEnv = process.env): ResolvedEmbeddingProviderConfig | null {
...
export function hostLocalConfigPaths(env: NodeJS.ProcessEnv = process.env): string[] {
  const paths: string[] = []
  ...
  if (process.platform === 'win32') {
    ...
    if (appData) paths.push(path.join(appData, 'llm-wiki', 'config.json'))
  } else {
...
export function normalizeProviderName(value: string): EmbeddingProviderConfig['provider'] {
...
export function parseFormat(value: string): EmbeddingRequestFormat {
...
export function defaultFormatForProvider(provider: EmbeddingProviderConfig['provider'], endpoint: string): EmbeddingRequestFormat | undefined {
...
export function inferFormat(endpoint: string): EmbeddingRequestFormat {
...
function readHostLocalEmbeddingConfig(env: NodeJS.ProcessEnv): { embedding: HostLocalEmbeddingConfig; configPath: string } | null {
...
function parseOptionalPositiveInteger(value: unknown, envName: string): number | undefined {
