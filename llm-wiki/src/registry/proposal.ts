...
import { buildClassificationPackage, extractProfileTerms } from './classification.js'
...
export async function classifyRouteProposal(
  paths: RegistryPaths,
...
> & { intakeItemId: string | null }> {
  ...
  const profileSuggestion = await createProfileProposalForSource(paths, {
    source,
    sourcePath: intakeItem ? path.join(paths.root, (intakeItem as { currentPath?: string }).currentPath ?? source.title) : source.title,
    intakeItemId: intakeItem?.id ?? null,
...
export async function createProfileProposalForSource(paths: RegistryPaths, input: {
  source: SourceSummary
  sourcePath: string
  intakeItemId: string | null
...
export function evaluateNewWikiCriteria(source: SourceSummary, candidates: RouteCandidate[]): ProfileProposal['newWikiCriteria'] {
...
export function buildClassificationPolicy(criteria: ProfileProposal['newWikiCriteria']): RouteProposal['classificationPolicy'] {
  return {
...
export function buildRoutingAssessment(
...
export function routingAssessmentRationale(
  ownershipDecision: RouteProposal['routingAssessment']['ownershipDecision'],
  top: RouteCandidate,
): string {
...
export function suggestWikiId(title: string, searchText: string): string {
...
export function taxonomySlugFromFile(filePath: string): string | null {
...
export async function updateIntakeItemAfterRouteAcceptance(
  paths: RegistryPaths,
...
export function markProfileProposalReviewed(
...
export async function readRouteDecisions(paths: RegistryPaths): Promise<Array<RouteDecision & { proposal: RouteProposal | null }>> {
