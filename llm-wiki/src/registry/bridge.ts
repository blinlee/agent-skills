...
import type { ChunkIndexStateV2 } from '../retrieval/types.js'
...
export async function runBridgeIndex(input: RegistryCommandInput): Promise<BridgeIndexResult> {
  ...
  for (const wiki of state.wikis) {
    ...
    for (const page of indexedPages) {
      ...
      for (const match of content.matchAll(BRIDGE_LINK_RE)) {
        ...
        const entry: CrossWikiLinkEntry = {
          fromWikiId: wiki.id,
  ...
  for (const edge of edges) {
    ...
    const edgeEntry: CrossWikiLinkEntry = {
...
export async function runBridgeTargets(input: BridgeTargetsInput): Promise<BridgeTargetsResult> {
  const paths = resolveRegistryPaths(input.registryRoot)
  await runRegistryInit({ registryRoot: paths.root })
  const state = await readRegistryState(paths)
  ...
  const candidates: BridgeTargetCandidate[] = []
...
export async function runBridgeCreateLanding(input: BridgeCreateLandingInput): Promise<BridgeDecisionResult> {
  const paths = resolveRegistryPaths(input.registryRoot)
  await runRegistryInit({ registryRoot: paths.root })
  const state = await readRegistryState(paths)
  const proposal = await readRequiredBridgeProposal(paths, input.proposalId)
...
async function acceptResolvedBridge(input: BridgeDecisionInput): Promise<BridgeDecisionResult> {
  const paths = resolveRegistryPaths(input.registryRoot)
  await runRegistryInit({ registryRoot: paths.root })
  const state = await readRegistryState(paths)
  const proposal = await readRequiredBridgeProposal(paths, input.proposalId)
...
async function readRequiredBridgeProposal(paths: RegistryPaths, proposalId: string): Promise<BridgeProposal> {
  const proposal = await readJsonFile<BridgeProposal | null>(bridgeProposalFile(paths, proposalId), null)
  if (!proposal) {
...
function bridgeEdgesFile(paths: RegistryPaths): string {
  return path.join(paths.bridgesDirectory, 'edges.json')
}

async function readBridgeEdges(paths: RegistryPaths): Promise<BridgeEdge[]> {
  const store = await readJsonFile<BridgeEdgesStore | null>(bridgeEdgesFile(paths), null)
  return store?.edges ?? []
}

async function upsertBridgeEdge(paths: RegistryPaths, edge: BridgeEdge): Promise<string> {
  ...
  } satisfies BridgeEdgesStore)
  return bridgeEdgesFile(paths)
}

async function resolveBridgeTarget(state: WikiRegistryState, proposal: BridgeProposal, rawTarget: string): Promise<ResolvedBridgeTarget> {
...
function parseBridgeTarget(rawTarget: string): { wikiId: string; target: string } {
...
function normalizeTargetSegment(raw: string, label: string): string {
...
function assertConcreteTargetSegment(segment: string): void {
...
function findWiki(state: WikiRegistryState, wikiId: string): WikiRegistryEntry {
...
async function bridgeLinkStatus(
  targetsByWiki: Map<string, WikiRegistryEntry>,
  toWikiId: string,
  toTarget: string,
): Promise<CrossWikiLinkEntry['status']> {
...
async function bridgeEdgeStatus(
  targetsByWiki: Map<string, WikiRegistryEntry>,
  edge: BridgeEdge,
): Promise<CrossWikiLinkEntry['status']> {
...
function normalizeRenderedTarget(rawTarget: string): string {
  return rawTarget.replace(/[.,;:]+$/g, '').replace(/\.md$/i, '')
}

function renderedBridgeKey(link: Pick<CrossWikiLinkEntry, 'fromWikiId' | 'fromTarget' | 'toWikiId' | 'toTarget'>): string {
  return [link.fromWikiId, link.fromTarget ?? '', link.toWikiId, link.toTarget].join('\u0000')
}

function edgeBridgeKey(edge: Pick<BridgeEdge, 'fromWikiId' | 'fromTarget' | 'toWikiId' | 'toTarget'>): string {
  return [edge.fromWikiId, edge.fromTarget ?? '', edge.toWikiId, edge.toTarget].join('\u0000')
}

async function renderedBridgeStatus(
  targetsByWiki: Map<string, WikiRegistryEntry>,
  edgeByKey: Map<string, BridgeEdge>,
  link: Pick<CrossWikiLinkEntry, 'fromWikiId' | 'fromTarget' | 'toWikiId' | 'toTarget'>,
): Promise<CrossWikiLinkEntry['status']> {
...
async function bridgeProposalSearchText(proposal: BridgeProposal): Promise<string> {
  let sourcePage = ''
...
async function readChunkIndex(knowledgeRoot: string): Promise<ChunkIndexStateV2 | null> {
  const chunksFile = path.join(knowledgeRoot, 'system', 'index', 'chunks.json')
  const parsed = await readJsonFile<Partial<ChunkIndexStateV2> | null>(chunksFile, null)
...
function dedupeTargetCandidates(candidates: BridgeTargetCandidate[]): BridgeTargetCandidate[] {
  const byTarget = new Map<string, BridgeTargetCandidate>()
...
async function appendBridgeLinkToSourcePage(sourcePageFile: string, link: string, rationale: string): Promise<void> {
...
async function appendLandingToIndex(knowledgeRoot: string, target: string, title: string): Promise<void> {
...
function titleFromTarget(slug: string): string {
  return slug
    .split(/[-_]/g)
    .filter(Boolean)
