...
import type { RouteAcceptInput, RouteAcceptResult, RouteInboxInput, RouteInboxResult, RouteInput, RouteProposal, RouteResult } from './registry.js'
...
export async function runRouteAccept(input: RouteAcceptInput): Promise<RouteAcceptResult> {
  ...
  const decisionStatus: 'accepted' | 'blocked' = ingestCompleted ? 'accepted' : 'blocked'
