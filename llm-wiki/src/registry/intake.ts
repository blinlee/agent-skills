...
export async function readIntakeItem(paths: RegistryPaths, itemId: string): Promise<IntakeItem | null> {
...
export async function writeIntakeItem(paths: RegistryPaths, item: IntakeItem): Promise<void> {
  await writeJsonFile(intakeItemFile(paths, item.id), item)
}

export function intakeItemFile(paths: RegistryPaths, itemId: string): string {
  return path.join(paths.intakeItemsDirectory, `${itemId}.json`)
}

export async function appendIntakeEvent(paths: RegistryPaths, value: unknown): Promise<void> {
  await appendJsonLine(paths.intakeEvents, value)
}

export async function findIntakeItemBySource(paths: RegistryPaths, source: string): Promise<IntakeItem | null> {
...
export async function findIntakeItemByRouteProposal(paths: RegistryPaths, proposalId: string): Promise<IntakeItem | null> {
  const items = await readIntakeItems(paths)
  return items.find((item) => item.routeProposalId === proposalId) ?? null
}

export function compareIntakeItems(left: IntakeItem, right: IntakeItem): number {
  return left.createdAt.localeCompare(right.createdAt) || left.id.localeCompare(right.id)
}

export function isTerminalIntakeStatus(status: IntakeItemStatus): boolean {
...
async function hashDirectory(root: string, hash: ReturnType<typeof createHash>, current: string): Promise<void> {
