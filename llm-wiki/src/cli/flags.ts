...
export function parseQueryArgs(args: string[]): QueryArgOptions {
  const questionParts: string[] = []
  let includeReview = false
  let disableHyde = false
  let full = false
  let readingMode: QueryReadingMode | undefined
...
export function parseCliFlags(args: string[]): Record<string, string[]> {
...
export function firstFlag(flags: Record<string, string[]>, key: string): string | undefined {
  return flags[key]?.[0]
}
