...
import {
  ...
  type ValidatedSemanticCuration,
} from '../compile/semantic-curation.js'
import { persistReviewItems } from '../governance/review.js'
import type { DedupEntry, DedupStore } from '../intake/dedup-store.js'
import { retainReviewableIntake } from '../intake/lifecycle.js'
import type { SourceKind } from '../types.js'

export type SemanticCurationPhaseResult =
  | { status: 'ready'; curationPlan: ValidatedSemanticCuration }
  | { status: 'needs_review'; reviewFiles: string[]; retainedPath: string | null; details: Record<string, unknown> }
...
export async function runSemanticCurationPhase(input: {
...
}): Promise<SemanticCurationPhaseResult> {
  ...
  const retainIfNeeded = async (): Promise<string | null> =>
    input.stagedPath ? retainReviewableIntake(input.stagedPath) : null
  ...
  let curationPlan: ValidatedSemanticCuration
  try {
