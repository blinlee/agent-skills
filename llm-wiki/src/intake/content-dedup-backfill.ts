...
import { createDedupStore, type DedupEntry } from './dedup-store.js'
import { loadContentDedupEmbedding } from './content-dedup-embedding.js'
import { backfillDedupManifestPageSnapshots, type DedupManifestSnapshotBackfillResult } from './manifest-migration.js'
import { readRawManifest, stripManagedRawFrontmatter, type RawManifestEntry } from './raw-store.js'
import type { SourceKind } from '../types.js'
import type { ChunkIndexStateV2 } from '../retrieval/types.js'

export type ContentDedupBackfillRecordResult = {
  sourceIdentity: string
  pageId: string | null
  title: string | null
  status: 'recorded' | 'skipped'
  reason?: 'no-source-page' | 'no-content' | 'duplicate-doc-hash' | 'already-current'
  docHash?: string
  chunkCount?: number
...
export type ContentDedupBackfillResult = {
  knowledgeRoot: string
  sourceManifest: DedupManifestSnapshotBackfillResult
  inspected: number
  recorded: number
  skipped: number
  embedded: number
  embeddingFailures: number
  records: ContentDedupBackfillRecordResult[]
}
...
export async function backfillContentDedupIndex(input: { knowledgeRoot: string }): Promise<ContentDedupBackfillResult> {
  ...
  const results: ContentDedupBackfillRecordResult[] = []
...
async function loadSourcePageCandidate(
  knowledgeRoot: string,
  entry: DedupEntry,
  rawEntries: Record<string, RawManifestEntry>,
): Promise<SourcePageCandidate | null> {
...
function resolveSourcePage(entry: DedupEntry): { pageId: string; relativePath: string; title?: string } | null {
...
async function readBestRawContent(
  knowledgeRoot: string,
  sourceIdentity: string,
  rawEntries: Record<string, RawManifestEntry>,
): Promise<string | null> {
...
async function loadSourceChunkCounts(knowledgeRoot: string): Promise<Map<string, number>> {
  try {
...
async function readMarkdownTitle(filePath: string): Promise<string | null> {
  try {
...
function rawEntryTime(entry: RawManifestEntry): string {
  return entry.archivedAt ?? entry.capturedAt ?? ''
}

function normalizeBackfillSourceKind(value: SourceKind): SourceKind {
