...
export type CreateTarGzArchiveInput = {
  sourceDir: string
  archivePath: string
}

export type CreateTarGzArchiveResult = {
  archivePath: string
  fileCount: number
}
...
export async function createTarGzArchive(input: CreateTarGzArchiveInput): Promise<CreateTarGzArchiveResult> {
...
async function listFiles(sourceDir: string, archivePath: string): Promise<string[]> {
  const output: string[] = []
...
function createTarHeader(input: { path: string; size: number; mtimeSeconds: number }): Buffer {
...
function splitTarPath(value: string): { name: string; prefix: string | null } {
