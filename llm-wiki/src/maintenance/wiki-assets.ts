...
import { createDedupStore, type DedupEntry, type SourceOutputManifest } from '../intake/dedup-store.js'
import { backfillDedupManifestPageSnapshots } from '../intake/manifest-migration.js'
import { readRawManifest, stripManagedRawFrontmatter, type RawManifestEntry } from '../intake/raw-store.js'
...
export type BackfillWikiAssetsResult = {
...
}): Promise<BackfillWikiAssetsResult> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const parsedArtifact = await parseBackfillArtifact({
        rawEntry: rawEntry as RawManifestEntry & { sourceKind: SourceKind },
        rawBody,
      })
...
function needsReadingBackfill(manifest: SourceOutputManifest): boolean {
...
function sourceSlugFromManifest(manifest: SourceOutputManifest): string | null {
...
function findRawEntryForDedupEntry(entry: DedupEntry, rawEntries: RawManifestEntry[]): RawManifestEntry | null {
...
function sourceRefsMatch(entry: DedupEntry, rawEntry: RawManifestEntry): boolean {
...
async function parseBackfillArtifact(input: {
  rawEntry: RawManifestEntry & { sourceKind: SourceKind }
  rawBody: string
...
async function buildBackfillSourcePageBody(input: {
  knowledgeRoot: string
  sourcePage: Pick<KnowledgePagePayload, 'slug'>
  generatedSourceBody: string
  readingPage: Pick<KnowledgePagePayload, 'slug'>
}): Promise<string> {
...
function buildMinimalSourcePageBody(
...
function sourceKindLabel(sourceKind: ParsedArtifact['sourceKind']): string {
  const labels: Record<ParsedArtifact['sourceKind'], string> = {
    md: 'Markdown',
    txt: '文本',
    url: '网页',
    repo: '代码仓库',
...
function preserveReviewFiles(current: SourceOutputManifest, previous: SourceOutputManifest): SourceOutputManifest {
  return {
    ...current,
