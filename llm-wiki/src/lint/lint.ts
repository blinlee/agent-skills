...
export async function runLint(input: LintCommandInput): Promise<LintCommandResult> {
  ...
  let indexedPages = [] as Awaited<ReturnType<typeof loadIndexedPages>>
...
async function lintTaxonomyGovernance(root: string): Promise<{ checkedFiles: string[]; errors: LintMessage[]; warnings: LintMessage[] }> {
  const paths = resolveKnowledgePaths(root)
  const checkedFiles = [paths.taxonomyCategoryGraph, paths.taxonomyRedirects]
  const errors: LintMessage[] = []
  const warnings: LintMessage[] = []

  const graph = await readJsonFile<{ nodes?: unknown[]; edges?: Array<{ from?: string; to?: string; type?: string; status?: string }> }>(paths.taxonomyCategoryGraph, { nodes: [], edges: [] })
  const edges = (graph.edges ?? []).filter((edge) => typeof edge.from === 'string' && typeof edge.to === 'string') as Array<{ from: string; to: string; type?: string; status?: string }>
  ...
  const cycle = findDirectedCycle(hierarchyEdges.map((edge) => [edge.from, edge.to] as const))
  if (cycle.length > 0) {
    errors.push({
      type: 'error',
      code: 'taxonomy-cycle',
  ...
  const redirects = await readJsonFile<{ redirects?: Record<string, string> }>(paths.taxonomyRedirects, { redirects: {} })
...
function findDirectedCycle(edges: ReadonlyArray<readonly [string, string]>): string[] {
...
async function collectManagedRawFiles(root: string): Promise<string[]> {
  const rawRoot = path.join(root, 'raw')
  const results: string[] = []
  for (const directory of ['staged', 'archive'] as const) {
...
async function lintWikiPageQuality(wikiFiles: string[]): Promise<{ errors: LintMessage[]; warnings: LintMessage[] }> {
  const errors: LintMessage[] = []
  const warnings: LintMessage[] = []
  ...
  for (const filePath of wikiFiles) {
    ...
    if (!frontmatter) {
      ...
      for (const field of ['title', 'created', 'updated', 'type', 'tags', 'sources'] as const) {
...
function resolveWikiPageKind(filePath: string, frontmatter: Record<string, unknown> | null): string {
...
function extractWikiTitle(content: string, frontmatter: Record<string, unknown> | null): string | null {
...
function lintScalingThresholds(indexedPages: Awaited<ReturnType<typeof loadIndexedPages>>): LintMessage[] {
  const warnings: LintMessage[] = []
...
function parseWikiFrontmatter(content: string): Record<string, unknown> | null {
...
async function resolveLintLink(
  root: string,
  rawTarget: string,
  indexedPages: Awaited<ReturnType<typeof loadIndexedPages>>,
): Promise<
  | { status: 'resolved'; target: string; filePath: string }
  | { status: 'ambiguous'; matches: Array<{ target: string; filePath: string }> }
  | { status: 'missing' }
> {
...
function toDiskPage(root: string, filePath: string): { target: string; filePath: string } | null {
...
async function collectWikiMarkdownFiles(root: string): Promise<string[]> {
