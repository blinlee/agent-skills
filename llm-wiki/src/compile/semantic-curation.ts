...
export type SemanticCurationStatus = 'ready' | 'needs_review'
...
export type SemanticCurationPlan = {
  schema: 'llm-wiki.semantic-curation.v1'
  status: SemanticCurationStatus
  summary: string
  entities: CuratedEntity[]
  concepts: CuratedConcept[]
  syntheses?: CuratedSynthesis[]
  rejections?: CuratedRejection[]
  notes?: string[]
}

export type ValidatedSemanticCuration = SemanticCurationPlan & {
  syntheses: CuratedSynthesis[]
  rejections: CuratedRejection[]
  notes: string[]
}

export async function loadSemanticCurationPlan(filePath: string): Promise<SemanticCurationPlan> {
  const parsed = JSON.parse(await readFile(filePath, 'utf8')) as unknown
  return normalizeSemanticCurationPlan(parsed)
}

export function normalizeSemanticCurationPlan(value: unknown): SemanticCurationPlan {
  if (!isRecord(value)) {
...
export function validateSemanticCurationPlan(input: {
  ...
  plan: SemanticCurationPlan
}): ValidatedSemanticCuration {
  const plan: ValidatedSemanticCuration = {
    ...input.plan,
  ...
  for (const rejection of plan.rejections) {
    if (!rejection.text.trim()) errors.push('rejection text must not be empty')
    if (!rejection.reason.trim()) errors.push(`rejection ${rejection.text || '<empty>'} must explain the reason`)
  }
...
export function semanticCurationNeedsReviewReasons(plan: SemanticCurationPlan): string[] {
...
function validateCuratedNode(
  section: 'entity' | 'concept' | 'synthesis',
  item: CuratedEntity | CuratedConcept | CuratedSynthesis,
  artifact: NormalizedArtifact,
  scopedSlugs: Set<string>,
  errors: string[],
): void {
  if (!item.title.trim()) errors.push(`${section} title must not be empty`)
  if (!item.description.trim()) errors.push(`${section} ${item.title || '<empty>'} needs a Chinese description`)
...
function normalizeEntities(value: unknown): CuratedEntity[] {
  return normalizeArray(value, 'entities').map((item, index) => {
    if (!isRecord(item)) throw new Error(`entities[${index}] must be an object`)
    return {
...
function normalizeConcepts(value: unknown): CuratedConcept[] {
...
function normalizeSyntheses(value: unknown): CuratedSynthesis[] {
  return normalizeArray(value, 'syntheses').map((item, index) => {
    if (!isRecord(item)) throw new Error(`syntheses[${index}] must be an object`)
    return {
...
function normalizeRejections(value: unknown): CuratedRejection[] {
  return normalizeArray(value, 'rejections').map((item, index) => {
    if (!isRecord(item)) throw new Error(`rejections[${index}] must be an object`)
    return {
...
function normalizeEvidence(value: unknown, path: string): SemanticEvidence[] {
...
function normalizeEntityKind(value: unknown): CuratedEntityKind {
  const allowed = new Set<CuratedEntityKind>([
    'paper',
    'system',
    'model',
    'dataset',
  ...
  if (typeof value !== 'string' || !allowed.has(value as CuratedEntityKind)) {
    throw new Error(`entity kind must be one of ${[...allowed].join(', ')}`)
  }
  return value as CuratedEntityKind
}
...
function normalizeForQuoteSearch(value: string): string {
