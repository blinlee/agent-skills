...
export async function applySourceSemanticLinks(input) {
    ...
    const relatedTargets = rankRelatedSources({ ...input.source, content: currentContent }, existingSources).slice(0, 3);
...
export async function pruneMissingSourceSemanticLinks(knowledgeRoot) {
...
async function listExistingSourcePages(sourceDirectory, currentSlug) {
    const entries = await readdir(sourceDirectory, { withFileTypes: true });
    const pages = [];
...
function pruneRelatedWikiPagesSection(content, currentSlug, existingSourceSlugs) {
...
function rankRelatedSources(current, candidates) {
...
function isStrongDomainTerm(term) {
    return term.length >= 7 || /[0-9]/.test(term);
}
function appendSectionEntries(content, heading, entries) {
...
function formatSourceLink(target) {
