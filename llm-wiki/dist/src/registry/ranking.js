...
export function rankWikis(searchText, wikis, focusText = searchText) {
...
export function collectRouteEvidence(wiki, sourceTokens, normalizedSearchText) {
...
export function matchedProfilePhrases(phrases, normalizedSearchText) {
...
export function matchedProfileTokens(phrases, sourceTokens) {
    return [...new Set(phrases.flatMap(tokenize).filter((term) => sourceTokens.has(term)))];
}
export function routeTokenSet(value) {
    return new Set(tokenize(value).flatMap(routeTokenVariants));
}
export function routeTokenVariants(token) {
...
export function routeMatchQuality(score, evidence) {
    if (score <= 0) {
        return 'none';
    }
...
export function routeRelationshipHint(score, evidence, focusedEvidence) {
    if (score <= 0) {
        return 'unrelated';
    }
    if (score >= ROUTE_STRONG_MATCH_SCORE && hasStrongRouteEvidence(evidence)) {
...
export function routeFocusedMatches(evidence) {
    return [...new Set([
            ...evidence.phraseMatches,
            ...evidence.aliasMatches,
            ...evidence.coreMatches,
...
export function focusedRouteText(source) {
    return `${source.title}\n${source.excerpt}`;
}
export function profilePositiveTerms(wiki) {
    return [...new Set([
            wiki.id,
            wiki.title,
            ...wiki.aliases,
...
export function buildBridgeSuggestions(candidates) {
