...
import { STOP_WORDS, titleFromId, tokenize } from './helpers.js';
...
export async function buildClassificationPackage(paths, input) {
...
export function buildPackageTopics(source) {
...
export function extractClassificationTopicTerms(source, limit) {
    return [
...
export function extractTitleTopicCandidates(title) {
...
export function normalizeClassificationTitle(title) {
    return title
        .replace(/^#+\s*/, '')
...
export function splitClassificationTitleSegments(title) {
    return title
        .split(/\s+(?:[-–—:])\s+|[:：]/g)
...
export function addTitleTopicCandidate(candidates, rawPhrase) {
    const slug = slugifyClassificationTopic(rawPhrase);
    if (isUsefulClassificationTopic(slug)) {
        candidates.add(slug);
    }
}
export function slugifyClassificationTopic(value) {
    return value
        .toLowerCase()
        .replace(/[^\p{L}\p{N}-]+/gu, ' ')
        .split(/\s+/g)
...
export function isUsefulClassificationTopic(slug) {
...
export function isUsefulClassificationTopicToken(token) {
...
export async function collectRelatedPages(source, wikis) {
...
export function buildLinkSuggestions(primaryWikiId, relatedPages, bridgeSuggestions) {
...
export function buildClassificationOperations(paths, routeProposalId, intakeItemId, newWikiProposalId, decisionType, recommendedWikiId) {
    const operations = [];
...
export function extractProfileTerms(searchText, title, limit) {
