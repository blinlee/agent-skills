...
const MAX_TOPICS = 8;
export async function analyzeArtifact(artifact) {
...
function buildSourceSummary(title, content) {
    const lead = content
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean)
...
function extractMarkerCandidates(lines, pattern, source, confidence) {
    const candidates = [];
...
function extractHeuristicEntities(artifact, content) {
...
function collectHeuristicEntityMatches(title, content) {
...
function summarizeHeuristicEntityMatches(matches) {
    const stats = new Map();
    for (const match of matches) {
...
function isAllowedHeuristicEntityMatch(match, sourceKind, stats) {
...
function isGenericRepoEntityPhrase(value) {
    const tokens = value
        .toLowerCase()
        .split(/[-_\s]+/)
        .filter(Boolean);
...
function hasBrandLikeSignal(value) {
    return value.split(/[-_\s]+/).some((token) => /[a-z][A-Z]/.test(token) || /\d/.test(token));
}
function unwrapMarkdownHeading(line) {
    const match = line.match(/^#{1,6}\s+(.+)$/);
    return match?.[1]?.trim() ?? null;
}
function isSentenceLeadingMatch(line, matchIndex) {
    const prefix = line.slice(0, matchIndex).trimEnd();
    return prefix.length === 0 || /[.!?]\s*$/.test(prefix);
}
function extractHeuristicConcepts(artifact, content) {
    const candidates = extractTitleConceptCandidates(artifact.title);
...
function countConceptTerms(text) {
...
function extractTitleConceptCandidates(title) {
...
function buildTopicProposals(title, entities, concepts) {
    const proposals = [];
...
function buildRelationHints(entities, concepts, title) {
...
function buildReviewTriggers(entities, concepts, topics, options = {}) {
    const triggers = [];
...
function isSourceMapArtifact(artifact, content) {
...
function computeOverallConfidence(entities, concepts, topics) {
...
function deriveControlledTitlePhrases(title) {
    const cleanTitle = normalizeTitleForLabels(title);
    const phraseSet = new Map();
...
function normalizeTitleForLabels(title) {
    return title
        .replace(/^#+\s*/, '')
...
function addControlledPhrase(phraseSet, rawPhrase) {
...
function normalizeControlledPhrase(rawPhrase) {
...
function removeAdjacentRootDuplicates(tokens) {
    const result = [];
...
function stripAnalyzerBoilerplate(value) {
    return value
        .split('\n')
