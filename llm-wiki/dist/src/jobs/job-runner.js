...
function compareCompiledAt(left, right) {
...
function normalizeSourceIdentity(sourceKind, source) {
    return sourceKind === 'url' ? source.trim() : path.resolve(source);
}
function buildStableSourceId(sourceKind, sourceIdentity) {
    return hashSourceMetadata({ sourceKind, sourceIdentity }).slice(0, 16);
}
function resolveCollisionFreeSourceSlug(baseSlug, artifactId, otherEntries) {
...
function resolveFinalStatus(hasReviewTriggers) {
