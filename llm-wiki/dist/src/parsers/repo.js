...
import { createParsedArtifact, normalizeTextBody } from './base.js';
...
export async function parseRepoSource(input) {
    ...
    const sampleCandidateFiles = allFiles.filter((filePath) => !isReadmeFile(filePath) && !isDocFile(filePath) && isSampleCandidate(filePath));
...
async function listRepoFiles(rootDirectory, relativeDirectory = '') {
...
function isReadmeFile(relativePath) {
...
function isSampleCandidate(relativePath) {
    const extension = path.posix.extname(relativePath).toLowerCase();
    return TEXT_FILE_EXTENSIONS.has(extension);
}
function prioritizeShallowPaths(relativePaths) {
    return [...relativePaths].sort((left, right) => {
        const depthDifference = left.split('/').length - right.split('/').length;
        return depthDifference === 0 ? left.localeCompare(right) : depthDifference;
    });
}
function selectFilesForCapture(relativePaths, limit) {
...
async function loadSnapshot(rootDirectory, relativePath, category) {
...
function buildOmissionMarkers(omissions) {
    return [
...
function formatOmissionMarker(label, omittedCount) {
    if (omittedCount <= 0) {
        return undefined;
    }
