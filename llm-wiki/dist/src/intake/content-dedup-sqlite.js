...
export function readSqliteState(dbPath) {
    ...
    try {
        ...
        const records = db.prepare('SELECT * FROM dedup_index ORDER BY doc_hash').all();
        const logs = db.prepare('SELECT * FROM dedup_log ORDER BY created_at, id').all();
        const pendingDecisions = db.prepare('SELECT * FROM dedup_pending_decisions ORDER BY created_at, id').all();
        return {
            version: 1,
...
export function writeSqliteState(dbPath, state) {
...
function recordToSqliteRow(record) {
    return {
...
function recordFromSqliteRow(row) {
    return {
...
function pendingDecisionToSqliteRow(decision) {
    return {
        id: decision.id,
...
function pendingDecisionFromSqliteRow(row) {
    return {
        id: row.id,
...
function normalizePendingReason(value) {
