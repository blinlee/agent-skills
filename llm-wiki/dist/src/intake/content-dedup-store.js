...
export function contentDedupIndexPath(knowledgeRoot) {
    return contentDedupDatabasePath(knowledgeRoot);
}
export function contentDedupDatabasePath(knowledgeRoot) {
    return path.join(path.resolve(knowledgeRoot), 'system', 'dedup', 'content-index.db');
}
export function normalizeContentForDedup(content) {
    return content
        .replace(/\r\n?/g, '\n')
        .split('\n')
...
export function contentHashForDedup(content) {
    return createHash('sha256').update(normalizeContentForDedup(content)).digest('hex');
}
export function createContentDedupStore(statePath) {
    ...
    return {
        async check(input) {
            ...
            const candidates = state.records.flatMap((record) => {
        ...
        async createPendingDecision(input) {
            return withWriteLock(async () => {
                const state = await readState(statePath, dbPath);
                const existing = state.pendingDecisions.find((decision) => decision.status === 'pending'
                    && decision.newDocHash === input.docHash
                    && decision.sourceIdentity === input.sourceIdentity
        ...
        async mergeRecords(input) {
            return withWriteLock(async () => {
                ...
                state.pendingDecisions = state.pendingDecisions.map((decision) => (decision.matchedPageId === sourcePageId
...
async function appendLog(statePath, input, withWriteLock, dbPath) {
...
async function readState(statePath, dbPath) {
...
function contentDedupDbPathForStatePath(statePath) {
    return statePath;
}
function emptyState() {
...
function pendingReasonForCandidate(candidate) {
...
function scanContentDedupCandidates(records) {
    const candidates = [];
...
function resolveSingleRecordReference(records, reference, role) {
    const normalizedReference = normalizeRecordReference(reference);
    const matches = records.filter((record) => record.docHash === reference
...
function normalizeRecordReference(reference) {
...
function semanticSimilarityForRecords(left, right) {
    if (!left.embeddingVector
