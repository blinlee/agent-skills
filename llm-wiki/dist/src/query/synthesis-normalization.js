...
export function normalizeStoredSuggestion(raw) {
...
function normalizeGrounding(value, suggestionId) {
    ...
    const citedChunkIds = Array.isArray(candidate.citedChunkIds)
        ? candidate.citedChunkIds.filter((item) => typeof item === 'string')
        : [];
    return {
        answerability,
...
function normalizeGroundingClaims(value) {
    ...
    return value
        ...
        supportingCitations: Array.isArray(item.supportingCitations) ? item.supportingCitations.filter((index) => typeof index === 'number') : [],
        citationIndexes: Array.isArray(item.citationIndexes) ? item.citationIndexes.filter((index) => typeof index === 'number') : [],
        chunkIds: Array.isArray(item.chunkIds) ? item.chunkIds.filter((chunkId) => typeof chunkId === 'string') : [],
        ...
        matchedTerms: Array.isArray(item.matchedTerms) ? item.matchedTerms.filter((term) => typeof term === 'string') : [],
...
function normalizeGroundingConflicts(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .filter((item) => typeof item === 'object' && item !== null)
        ...
        targets: Array.isArray(item.targets) ? item.targets.filter((target) => typeof target === 'string') : [],
        chunkIds: Array.isArray(item.chunkIds) ? item.chunkIds.filter((chunkId) => typeof chunkId === 'string') : [],
    }));
}
function normalizeClaimValidation(value, fallback) {
    const record = typeof value === 'object' && value !== null ? value : fallback;
    ...
    return {
        ...
        matchedTerms: Array.isArray(record.matchedTerms) ? record.matchedTerms.filter((term) => typeof term === 'string') : [],
        citationCoverage: typeof record.citationCoverage === 'number' ? record.citationCoverage : 0,
        supportLevel,
    };
}
function normalizeContradictionTable(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .filter((item) => typeof item === 'object' && item !== null)
...
function normalizeEvidencePair(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value
...
function normalizeConflictEvidenceList(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value
...
function normalizeConflictEvidence(value) {
    if (typeof value !== 'object' || value === null) {
        return null;
    }
    const item = value;
