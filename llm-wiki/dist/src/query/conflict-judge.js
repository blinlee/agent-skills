export class HeuristicEvidenceConflictJudge {
    patterns = [
    ...
    findSignals(citations) {
        ...
        citations.forEach((citation, index) => {
            ...
            const evidence = [selfEvidence, pairedEvidence].filter((entry) => entry !== null);
            signals.push({
                ...
                chunkIds: [...new Set(evidence.map((entry) => entry.chunkId).filter((chunkId) => Boolean(chunkId)))],
            });
        });
        return signals;
    }
}
export function buildContradictionTable(conflicts) {
    return conflicts
...
function conflictEvidence(citation, index, matchedText) {
    return {
        citationIndex: index + 1,
        target: citation.target,
        chunkId: citation.chunkId,
...
function nearestEvidencePair(citations, index) {
...
function contradictionSummary(signal) {
...
function contradictionRecommendation(signal) {
    if (signal.kind === 'stale') {
