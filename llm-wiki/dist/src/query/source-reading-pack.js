...
export async function buildSourceReadingPack(knowledgeRoot, answerability, citations, redactor = (text) => text) {
    const indexState = await loadChunkIndexForSourceReading(knowledgeRoot);
    const passages = citations.map((citation, index) => buildSourceReadingPassage(indexState, citation, index, redactor));
    return {
        answerability,
        readingMode: 'passage',
...
async function loadChunkIndexForSourceReading(knowledgeRoot) {
    try {
...
function buildSourceReadingPassage(indexState, citation, index, redactor) {
...
function sourceReadingPassageFromCitation(citation, index, redactor) {
...
function expandSourcePassageFromChunk(indexState, chunk) {
    if (!indexState) {
        return null;
    }
...
function stitchNeighborChunks(indexState, chunk) {
    const candidates = [
...
function appendWithOverlap(current, next) {
...
function normalizeSourcePassageText(value) {
    return value.replace(/\r\n?/g, '\n').replace(/\n{4,}/g, '\n\n\n').trim();
}
function compactSourcePassageText(value, maxChars) {
