export const DEFAULT_QUERY_INTENT_PROFILES = {
    finance: {
        domain: 'finance',
        core: [
            'quant',
...
export function buildQueryIntent(question, profiles = Object.values(DEFAULT_QUERY_INTENT_PROFILES)) {
    ...
    const focusedDomains = new Set([...domains].filter((domain) => containsAny(normalized, profilesByDomain[domain]?.focus ?? [])));
...
export function isFinanceQuantEvidenceForIntent(intent, evidence) {
...
function classifyAnswerShape(normalizedQuestion) {
