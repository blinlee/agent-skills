...
export function buildGroundingDiagnostics(question, selection, citations, evidenceBudget, lowRetrievalConfidence = false, conflictJudge = new HeuristicEvidenceConflictJudge(), queryIntent = buildQueryIntent(question)) {
    ...
    return {
        ...
        citedChunkIds: citations.map((citation) => citation.chunkId).filter((chunkId) => Boolean(chunkId)),
        claims,
        conflicts,
        contradictionTable,
    };
}
function citationsHaveQuestionSupport(question, citations) {
    const stop = new Set([
...
function citationsHaveSemanticQuestionSupport(question, citations, intent) {
    const rawSemanticCitations = citations.filter((citation) => {
        ...
        const hasSemanticReason = citation.retrievalReasons?.some((reason) => reason.startsWith('embedding:cosine:') || reason === 'coverage:semantic-signal') ?? false;
        return embeddingScore >= 0.25 || rerankScore >= 0.35 || hasSemanticReason;
    });
    if (rawSemanticCitations.length === 0) {
        return false;
    }
    const multilingualMismatch = containsCjk(question) && rawSemanticCitations.some((citation) => mostlyLatin(`${citation.title} ${citation.heading ?? ''} ${citation.excerpt}`));
...
function buildGroundedClaims(question, citations) {
...
function extractSemanticClaimUnits(excerpt) {
    const seen = new Set();
    const claims = [];
...
function splitClaimSentences(excerpt) {
...
function rankClaimSentences(sentences, questionTokens) {
    return sentences
        .map((sentence, index) => ({
        sentence,
        index,
...
function scoreClaimSentence(sentence, questionTokens) {
...
function validateClaimAgainstCitation(sentence, questionTokens, citation) {
...
function meaningfulClaimTokens(sentence) {
...
function buildClaimReason(sentence, questionTokens, citation, validation) {
    const sentenceTokens = new Set(tokenize(sentence));
...
function normalizeClaimKey(sentence) {
    return compact(sentence).toLowerCase();
}
function strongestSupportLevel(left, right) {
    const rank = { weak: 0, partial: 1, strong: 2 };
