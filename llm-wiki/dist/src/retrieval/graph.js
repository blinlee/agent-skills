...
export function scoreGraphBoosts(input) {
    ...
    const boostsByPage = new Map();
    const firstHopTargets = new Set();
...
function outgoingResolvedTargets(pageTarget, graph) {
    return graph.links
        .filter((link) => link.from === pageTarget && link.status === 'resolved' && link.to)
        .map((link) => link.to);
}
function addPageBoost(boosts, pageTarget, score, reason) {
