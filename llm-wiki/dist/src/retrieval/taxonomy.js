...
function matchedQueryTopics(taxonomy, queryTokens) {
...
function categoryEdgeBoosts(edges, matchedTopics) {
    const boosts = [];
...
function categoryGraphTopicSlugs(edges) {
    const slugs = new Set();
    for (const edge of edges) {
        if (edge.status !== undefined && edge.status !== 'accepted') {
            continue;
...
function mergeBoost(boosts, topicSlug, boost) {
...
function canonicalTopicSlug(slug, redirects) {
    let current = slug;
    const seen = new Set();
    for (let i = 0; i < 8; i += 1) {
...
function tokensMatchSlugOrName(query, slug, name) {
