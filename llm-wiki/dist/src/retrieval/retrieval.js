...
import { evidenceBudgetForChunkCount } from './context-budget.js';
...
function mergeGraphBoosts(left, right) {
...
function buildSignalSummary(mode, hits, evidenceBudget) {
    return {
        mode,
...
async function loadQueryEmbeddingIfAvailable(knowledgeRoot, question, diagnostics, options = {}) {
    ...
    try {
        ...
        diagnostics.push(`query embedding unavailable; using lexical retrieval only: ${error.message}`);
        return { config, queryVector: null, records: new Map() };
    }
}
