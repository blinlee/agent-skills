...
describe('buildCli', () => {
  ...
  it('ships a generic skill contract that delegates to the CLI surface', async () => {
    ...
    expect(skill).toContain('operate an llm-wiki knowledge root or atlas registry through the package CLI')
    expect(skill).toContain('references/command-map.md')
    expect(skill).toContain('references/classification-review.md')
    expect(skill).toContain('references/embedding-provider.md')
    expect(skill).toContain('scripts/query_handoff.py')
    expect(skill).toContain('/anything2md')
    expect(skill).toContain('Hard gate: do not run')
    expect(skill).toContain('route-accept')
    expect(skill).toContain('save-synthesis --confirm')
    expect(skill).not.toContain('/llm-wiki review')
    expect(skill).toContain('the user explicitly approves the proposed action')
