...
export const NotificationTestPanel: React.FC<NotificationTestPanelProps> = ({
  ...
  const [title, setTitle] = useState('invest-analysis-pro 通知测试');
  const [content, setContent] = useState('这是一条来自 invest-analysis-pro Web 设置页的通知测试消息。');
  ...
  const runTest = async () => {
    ...
    try {
      const payload = await systemConfigApi.testNotificationChannel({
        ...
        maskToken,
        title: title.trim() || 'invest-analysis-pro 通知测试',
        content: content.trim() || '这是一条来自 invest-analysis-pro Web 设置页的通知测试消息。',
