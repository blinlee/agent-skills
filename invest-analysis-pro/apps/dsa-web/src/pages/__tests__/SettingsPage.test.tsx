...
describe('SettingsPage', () => {
  beforeEach(() => {
    ...
    delete (window as { investAnalysisProDesktop?: unknown }).investAnalysisProDesktop;
  ...
  it('renders desktop app version in system settings during desktop runtime', async () => {
    (window as { investAnalysisProDesktop?: unknown }).investAnalysisProDesktop = { version: '3.12.0' };
  ...
  it('keeps version grid at three columns when desktop runtime has no usable version', async () => {
    (window as { investAnalysisProDesktop?: unknown }).investAnalysisProDesktop = { version: '   ' };
  ...
  it('ignores non-string desktop runtime version values without breaking render', async () => {
    (window as { investAnalysisProDesktop?: unknown }).investAnalysisProDesktop = { version: 3120 };
  ...
  it('normalizes malformed desktop update payloads instead of throwing', async () => {
    ...
    (window as { investAnalysisProDesktop?: unknown }).investAnalysisProDesktop = createDesktopRuntime();
  ...
  it('renders desktop update notice when a newer release is available', async () => {
    desktopGetUpdateState.mockResolvedValue({
      ...
      releaseUrl: 'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0',
  ...
  it('opens GitHub release page from desktop update notice', async () => {
    ...
    await waitFor(() => {
      expect(desktopOpenReleasePage).toHaveBeenCalledWith(
        'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0'
      );
    });
  });
