...
const DEFAULT_GITHUB_REPOSITORY = process.env.GITHUB_REPOSITORY || 'invest-analysis-pro/invest-analysis-pro';
const [DEFAULT_GITHUB_OWNER, DEFAULT_GITHUB_REPO] = DEFAULT_GITHUB_REPOSITORY.split('/');
const GITHUB_OWNER = process.env.INVEST_ANALYSIS_PRO_GITHUB_OWNER || DEFAULT_GITHUB_OWNER || 'invest-analysis-pro';
const GITHUB_REPO = process.env.INVEST_ANALYSIS_PRO_GITHUB_REPO || DEFAULT_GITHUB_REPO || 'invest-analysis-pro';
...
const DESKTOP_UPDATE_BACKUP_DIR = '.invest-analysis-pro-update-backup';
const DESKTOP_UPDATE_BACKUP_MANIFEST_FILE = 'runtime-state.json';
const DESKTOP_UPDATE_RUNTIME_RELATIVE_FILES = Object.freeze([
  '.env',
  path.join('data', 'invest_analysis_pro.db'),
  path.join('data', 'invest_analysis_pro.db-wal'),
  path.join('data', 'invest_analysis_pro.db-shm'),
    ...
    const req = request(
      requestUrl,
      {
        ...
        headers: {
          ...
          'User-Agent': 'invest-analysis-pro-desktop',
        },
      },
      (incomingResponse) => {
...
function resolveBackendPath() {
  if (process.env.INVEST_ANALYSIS_PRO_BACKEND_PATH) {
    return process.env.INVEST_ANALYSIS_PRO_BACKEND_PATH;
  }

  if (app.isPackaged) {
    const backendDir = path.join(process.resourcesPath, 'backend');
    const exeName = isWindows ? 'invest_analysis_pro.exe' : 'invest_analysis_pro';
    const oneDirPath = path.join(backendDir, 'invest_analysis_pro', exeName);
...
function resolvePythonPath() {
  return process.env.INVEST_ANALYSIS_PRO_PYTHON || 'python';
}
...
function startBackend({ port, envFile, dbPath, logDir }) {
  ...
  const env = {
    ...process.env,
    INVEST_ANALYSIS_PRO_DESKTOP_MODE: 'true',
    ENV_FILE: envFile,
...
function isWindowsNsisInstalledApp() {
  ...
  return fs.existsSync(path.join(appDir, 'Uninstall invest-analysis-pro.exe'));
}
...
async function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      additionalArguments: [`--invest-analysis-pro-desktop-version=${app.getVersion()}`],
    },
  });
  ...
  const dbPath = path.join(appDir, 'data', 'invest_analysis_pro.db');
