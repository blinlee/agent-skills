...
test('preload exposes desktop version from BrowserWindow additionalArguments', (t) => {
  ...
  process.argv = [...originalArgv, `--invest-analysis-pro-desktop-version=${expectedVersion}`];
  ...
  assert.deepEqual(exposeInMainWorldCalls.map((call) => call[0]), ['investAnalysisProDesktop', 'dsaDesktop']);
  ...
  assert.equal(
    preloadModule.readDesktopVersion([`--invest-analysis-pro-desktop-version=${expectedVersion}`]),
    expectedVersion
  );
});
...
test('preload falls back to empty version when BrowserWindow does not pass one', (t) => {
  ...
  const preloadPath = require.resolve('../preload.js');
  delete require.cache[preloadPath];
  process.argv = originalArgv.filter((value) => !value.startsWith('--invest-analysis-pro-desktop-version='));
...
test('createDesktopBridge delegates update actions to ipcRenderer', async (t) => {
  ...
  assert.deepEqual(await desktopBridge.openReleasePage('https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0'), {
    channel: preloadModule.DESKTOP_OPEN_RELEASE_PAGE_CHANNEL,
    payload: 'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0',
  });
