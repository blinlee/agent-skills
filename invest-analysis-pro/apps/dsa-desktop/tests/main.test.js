...
function loadMainModule(t, options = {}) {
  ...
  const fakeApp = {
    isPackaged: false,
    getVersion: () => '3.12.0',
    getPath: () => '/tmp/invest-analysis-pro-user-data',
...
test('extractReleaseMetadata ignores releases without semver tags', (t) => {
  ...
  assert.equal(
    mainModule.extractReleaseMetadata({
      ...
      html_url: 'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/desktop-latest',
    }),
    null
  );
});
...
test('evaluateReleaseUpdate reports update-available when release is newer', (t) => {
  ...
  const state = mainModule.evaluateReleaseUpdate({
    ...
    release: {
      tag_name: 'v3.13.0',
      html_url: 'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0',
  ...
  assert.equal(state.releaseUrl, 'https://github.com/invest-analysis-pro/invest-analysis-pro/releases/tag/v3.13.0');
...
test('auto download prompt falls back to error when install path fails', async (t) => {
  const updaterEvents = {};
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'invest-analysis-pro-desktop-updater-'));
  const exeDir = path.join(tempRoot, 'app');
  const userDataDir = path.join(tempRoot, 'userData');
  const exePath = path.join(exeDir, 'invest-analysis-pro.exe');
  const uninstallPath = path.join(exeDir, 'Uninstall invest-analysis-pro.exe');
  const envFile = path.join(exeDir, '.env');
  const backupRoot = path.join(userDataDir, '.invest-analysis-pro-update-backup');
...
test('desktop update backup list includes WAL and SHM artifacts', (t) => {
  ...
  assert.ok(files.includes(path.join('data', 'invest_analysis_pro.db')));
  assert.ok(files.includes(path.join('data', 'invest_analysis_pro.db-wal')));
  assert.ok(files.includes(path.join('data', 'invest_analysis_pro.db-shm')));
...
test('restorePackagedRuntimeStateFromBackup keeps backup when copy fails', (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'invest-analysis-pro-desktop-restore-'));
  const appDir = path.join(tempRoot, 'app');
  const userDataDir = path.join(tempRoot, 'userData');
  const backupRoot = path.join(userDataDir, '.invest-analysis-pro-update-backup');
  const backupDbPath = path.join(backupRoot, 'data', 'invest_analysis_pro.db');
  fs.mkdirSync(path.dirname(backupDbPath), { recursive: true });
  fs.mkdirSync(appDir, { recursive: true });
  fs.writeFileSync(path.join(appDir, 'Uninstall invest-analysis-pro.exe'), '');
  fs.writeFileSync(backupDbPath, 'backup-db');
  fs.writeFileSync(
    path.join(backupRoot, 'runtime-state.json'),
    JSON.stringify({ files: [path.join('data', 'invest_analysis_pro.db')] }),
    'utf-8'
  );
  ...
  const mainModule = loadMainModule(t, {
    ...
    app: {
      ...
      getPath: (name) => {
        if (name === 'exe') {
          return path.join(appDir, 'invest-analysis-pro.exe');
        }
        return userDataDir;
      },
    },
...
test('restorePackagedRuntimeStateFromBackup removes restored files from pending manifest', (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'invest-analysis-pro-desktop-partial-restore-'));
  const appDir = path.join(tempRoot, 'app');
  const userDataDir = path.join(tempRoot, 'userData');
  const backupRoot = path.join(userDataDir, '.invest-analysis-pro-update-backup');
  ...
  const dbRelativePath = path.join('data', 'invest_analysis_pro.db');
...
test('restorePackagedRuntimeStateFromBackup skips backup when app version did not change', (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'invest-analysis-pro-desktop-same-version-restore-'));
  const appDir = path.join(tempRoot, 'app');
  const userDataDir = path.join(tempRoot, 'userData');
  const backupRoot = path.join(userDataDir, '.invest-analysis-pro-update-backup');
  const backupEnvPath = path.join(backupRoot, '.env');
...
test('createWindow startup path does not throw ReferenceError after restore result handling', async (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'invest-analysis-pro-desktop-startup-'));
  const appDir = path.join(tempRoot, 'app');
  const userDataDir = path.join(tempRoot, 'userData');
  const exePath = path.join(appDir, 'invest-analysis-pro.exe');
  const uninstallPath = path.join(appDir, 'Uninstall invest-analysis-pro.exe');
  const loadedFiles = [];
  const loadedUrls = [];
  let startupError;
  ...
  fs.mkdirSync(userDataDir, { recursive: true });
  fs.mkdirSync(backupRoot, { recursive: true });
  fs.mkdirSync(path.join(resourcesPath, 'backend', 'invest_analysis_pro'), { recursive: true });
  ...
  fs.writeFileSync(path.join(resourcesPath, 'backend', 'invest_analysis_pro', 'invest_analysis_pro.exe'), '');
