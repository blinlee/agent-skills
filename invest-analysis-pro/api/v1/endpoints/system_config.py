...
def _allow_env_backup_access(request: Request) -> None:
    ...
    - Desktop runtime keeps existing local behavior via INVEST_ANALYSIS_PRO_DESKTOP_MODE; DSA_DESKTOP_MODE is accepted as legacy compatibility.
    - Non-desktop runtime must have admin auth enabled and a valid session.
    """
    if os.getenv("INVEST_ANALYSIS_PRO_DESKTOP_MODE") == "true" or os.getenv("DSA_DESKTOP_MODE") == "true":
        return
