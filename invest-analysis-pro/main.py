...
def _setup_runtime_logging(log_dir: str, debug: bool = False) -> bool:
    ...
    try:
        setup_logging(log_prefix="invest_analysis_pro", debug=debug, log_dir=log_dir)
        return True
...
class _LazyPipelineDescriptor:
    ...
    def __get__(self, obj, objtype=None):
        if self._resolved is None:
            self._resolved = _get_analysis_pipeline()
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='invest-analysis-pro legacy standalone scheduler/API/WebUI entrypoint',
...
示例:
  ...
  python main.py invest-analysis-pro --help  # agent-native 数据 CLI（JSON evidence，不调用 LLM）
        '''
    )
...
def _maybe_run_agent_native_cli(argv: Optional[List[str]] = None) -> Optional[int]:
    """Dispatch invest-analysis-pro agent-native CLI without changing legacy flags.

    Accepted forms:
      python main.py invest-analysis-pro ...
      python main.py agent ...
      python main.py iap ...
    """
    argv = list(sys.argv[1:] if argv is None else argv)
    if not argv or argv[0] not in {"invest-analysis-pro", "agent", "iap"}:
        return None
    from src.agent.native_cli import main as agent_native_main

    return agent_native_main(argv[1:])
...
def main() -> int:
    ...
    Returns:
        退出码（0 表示成功）
    """
    agent_native_exit = _maybe_run_agent_native_cli()
    if agent_native_exit is not None:
        return agent_native_exit
    ...
    logger.info("=" * 60)
    logger.info("invest-analysis-pro 启动")
