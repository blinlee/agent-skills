...
from src.agent.tools.data_tools import _handle_get_capital_flow, _sum_recent_numeric
...
class TestGetCapitalFlowContract(unittest.TestCase):
    ...
    def test_recent_flow_sums_can_be_computed_from_daily_rows(self) -> None:
        """Agent-native fallback can derive 5/10 day inflow from daily AkShare rows."""
        df = pd.DataFrame({"主力净流入-净额": [float(i) for i in range(1, 13)]})

        self.assertEqual(_sum_recent_numeric(df, "主力净流入-净额", 5), 50.0)
        self.assertEqual(_sum_recent_numeric(df, "主力净流入-净额", 10), 75.0)
