...
if (Test-Path 'dist\invest_analysis_pro') {
  Remove-Item -Recurse -Force 'dist\invest_analysis_pro'
}

if (Test-Path 'build\invest_analysis_pro') {
  Remove-Item -Recurse -Force 'build\invest_analysis_pro'
}
...
if (!(Test-Path 'dist\invest_analysis_pro')) {
  throw 'PyInstaller finished but dist\invest_analysis_pro was not generated.'
}

Copy-Item -Path 'dist\invest_analysis_pro' -Destination 'dist\backend\invest_analysis_pro' -Recurse -Force

Write-Host 'Verifying static asset references (packaged)...'
$packagedStatic = Join-Path 'dist\backend\invest_analysis_pro' '_internal\static'
if (-not (Test-Path $packagedStatic)) {
  $packagedStatic = Join-Path 'dist\backend\invest_analysis_pro' 'static'
}
if (Test-Path $packagedStatic) {
  ...
  Write-Warning "Could not locate packaged static directory under dist\backend\invest_analysis_pro; skipping post-package check."
}
