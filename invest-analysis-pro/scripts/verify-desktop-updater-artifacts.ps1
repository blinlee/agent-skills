...
if ($releaseAssetsDirPath) {
  $installerFiles = Get-ChildItem -Path $releaseAssetsDirPath -Filter 'invest-analysis-pro-windows-installer-*.exe' -File -ErrorAction SilentlyContinue
  if (-not $installerFiles) {
    throw "No invest-analysis-pro-windows-installer-*.exe found in release assets: $releaseAssetsDirPath"
  }
} else {
