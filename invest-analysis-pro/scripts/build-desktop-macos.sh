...
if [[ ! -d "${ROOT_DIR}/dist/backend/invest_analysis_pro" ]]; then
  echo "Backend artifact not found: ${ROOT_DIR}/dist/backend/invest_analysis_pro"
...
install_desktop_dependencies() {
  ...
  package_lock_hash > node_modules/.invest-analysis-pro-package-lock.sha256
}

ensure_desktop_dependencies() {
  local marker="node_modules/.invest-analysis-pro-package-lock.sha256"
  local reason=""
...
MAC_ARCH="${INVEST_ANALYSIS_PRO_MAC_ARCH:-}"
ARCH_ARGS=()
if [[ -n "${MAC_ARCH}" ]]; then
  case "${MAC_ARCH}" in
    x64|arm64)
      ...
      ;;
    *)
      echo "Unsupported INVEST_ANALYSIS_PRO_MAC_ARCH: ${MAC_ARCH}. Use x64 or arm64."
      exit 1
      ;;
  esac
fi
