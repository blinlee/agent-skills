...
if [[ -d "${ROOT_DIR}/dist/invest_analysis_pro" ]]; then
  rm -rf "${ROOT_DIR}/dist/invest_analysis_pro"
fi

if [[ -d "${ROOT_DIR}/build/invest_analysis_pro" ]]; then
  rm -rf "${ROOT_DIR}/build/invest_analysis_pro"
fi
...
cmd=("${PYTHON_BIN}" -m PyInstaller --name invest_analysis_pro --onedir --noconfirm --noconsole --add-data "static:static" --collect-data litellm --collect-data tiktoken)
...
cp -R "${ROOT_DIR}/dist/invest_analysis_pro" "${ROOT_DIR}/dist/backend/invest_analysis_pro"

log "Verifying static asset references (packaged)..."
packaged_static="${ROOT_DIR}/dist/backend/invest_analysis_pro/_internal/static"
if [[ ! -d "${packaged_static}" ]]; then
  packaged_static="${ROOT_DIR}/dist/backend/invest_analysis_pro/static"
fi
if [[ -d "${packaged_static}" ]]; then
  "${PYTHON_BIN}" "${SCRIPT_DIR}/check_static_assets.py" "${packaged_static}"
else
  log "WARNING: could not locate packaged static directory under dist/backend/invest_analysis_pro; skipping post-package check."
fi
