...
$skipDevModeCheck = ($env:INVEST_ANALYSIS_PRO_SKIP_DEVMODE_CHECK -eq 'true') -or ($env:CI -eq 'true')
...
$backendArtifact = Join-Path $repoRoot 'dist\backend\invest_analysis_pro'
...
function Install-DesktopDependencies {
  ...
  Set-Content -Path 'node_modules\.invest-analysis-pro-package-lock.sha256' -Value (Get-PackageLockHash) -Encoding ascii
}

function Ensure-DesktopDependencies {
  $lockHashMarker = 'node_modules\.invest-analysis-pro-package-lock.sha256'
  $installReason = $null
...
Get-Process -Name "invest-analysis-pro" -ErrorAction SilentlyContinue | Stop-Process -Force
Get-Process -Name "invest_analysis_pro" -ErrorAction SilentlyContinue | Stop-Process -Force
