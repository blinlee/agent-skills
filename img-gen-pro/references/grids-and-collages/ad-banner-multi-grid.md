...
**单广告不能直接当矩阵 exemplar（反例提醒）**
> - "Transparent Labs Hydrate 健身补剂 Campaign"
> - "1980s Claude 复古杂志广告"
> - "品牌人格漫画信息图"
> 要点：这些案例可以提供“单格广告风格”的局部灵感，但它们本身不是多格矩阵，不应直接被当作本模板的 canonical exemplar。真正使用本模板时，应该把多个独立广告 panel 重新组织成统一 grid，而不是把一张完整海报强行塞进一个格子逻辑里。

### 吸收规则（社交媒体广告矩阵）

- 一张图里并排展示多个广告切角 / 多行业 banner / 多创意方向 → 留在本模板
- 研究库里若只有单张广告案例，可把它们当作“单格风格来源”，但不能当作整张矩阵的结构 exemplar
- 如果是同一主题的系列合集、九宫格、lookbook → 改走 `lookbook-grid.md`
- 如果只是单张高冲击社媒封面 → 改走 `short-video-cover-ui.md`
