...
适用于游戏角色动作库、格斗 / 舞蹈 pose 参考、动画前期姿势字典。若重点是“同一角色做很多动作”而不是完整设定，应优先用这个变体而不是硬塞进三视图。
...
```json
{
  ...
  "sections": {
    ...
    "pose_grid": {
      "enabled": true,
      "count": "{argument name=\"pose count\" default=\"16\"}",
      "theme": "{argument name=\"pose theme\" default=\"战斗 / 街舞 / 跑酷 / 待机动作\"}",
      "annotations": "{argument name=\"annotations\" default=\"编号 + 简短动作说明 + 方向箭头\"}"
    }
  },
  "constraints": {
    "must_keep": [
      "同一角色面相、服装、镜头距离一致",
      "每格只改变 body language，不换角色",
      "背景保持纯净，优先给动作细节留像素"
    ],
    "avoid": [
      "把动作参考板做成剧情分镜",
...
**街舞角色设定参考图**
> Prompt: "角色设定图布局，聚焦于一位 18 岁的亚裔女性街舞舞者。包含 4 个大型高细节全身动态舞姿，侧边附 3 个头部特写，最大限度减少文字元素，将像素优先给面部和动作细节。"
> 要点：这类并不是传统三视图，而是“角色 identity + 动作语汇”的混合板。适合吸收到角色设定模板的动作参考变体，而不是单独新建游戏角色类别。

**4×4 动作分解参考表**
> Prompt: "4×4 grid layout with a total of 16 panels... the same character appears consistently in all panels... directional arrows indicating movement... game concept art quality."
> 要点：当任务核心是动作库、招式表、动画参考时，要优先锁“同一角色一致性”和“每格动作差异”，文字标注只做辅助，不要反客为主。
