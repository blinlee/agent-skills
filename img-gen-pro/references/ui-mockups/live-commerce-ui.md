...
- 用户要做直播带货截图、主播带货样机、评论区 / 礼物区 / 商品卡叠加界面
- 用户强调要像真实平台直播截图，而不是纯 UI 设计稿或纯摄影海报
- 用户明确提到抖音 / 小红书 / 淘宝直播 / 通用中文直播平台语法

不要使用：

- 只是普通社媒帖子详情页、评论区页面 → 用 `social-interface-mockup.md`
- 只是直播预告封面或短视频缩略图 → 用 `short-video-cover-ui.md`
- 只是落地页 hero 商品卡，不需要直播语法 → 用 `product-card-overlay.md`
...
- **必问**：主播来源（真人照片 / 名人名字 / 人物描述 / 随机生成）、商品类型 / 商品名、平台风格、界面语言
- **可默认**：礼物文案、弹幕条数、顶部观众统计、背景品牌元素、商品卡热卖标签
- **可随机**：次级评论内容、礼物昵称、浮层细节、次级 icon 排列与轻装饰

### 自动补全策略

- 用户只给主播，不给商品 → 自动补齐与人设相容的商品、价格区间与商品卡文案
- 用户只给商品，不给主播 → 自动补一个适合镜头表达的主播人设，并保持直播平台语法成立
- 用户只说“做直播截图” → 先问 2-4 个最高影响字段，其余次要 UI 内容自动补齐
- 用户明确说“自动补全 / 你来定” → 保留平台、比例、直播语法为强约束，其余按模板默认值或合理随机生成

## 避免事项

- 不要把直播截图做成纯平 UI 设计稿，缺少真人画面与平台叠加层融合
- 不要让弹幕、商品卡、礼物区遮挡主播核心脸部或手持主体
- 不要混搭多个平台语法（例如抖音直播 UI + 小红书帖子结构）
- 不要生成与主播人设明显不协调的商品、礼物文案或评论语气
- 不要让商品卡、价格、CTA、弹幕完全不可读
...
以下是来自社区的真实使用案例，展示了本模板的典型使用场景：

**刘亦菲抖音直播截图**
> Prompt: "9:16 的图片比例，生成一张抖音直播的截图，里面是刘亦菲在直播，刘亦菲手里拿着牌子，牌子里写着 今晚直播，欢迎来参亦菲畅聊！"
> 要点：名人+直播间+手持道具（牌子/商品）是高频用法。直播 UI 必须与目标平台一致。

**马斯克抖音直播截图（极详细 UI 描述范例）**
> Prompt: "A 9:16 vertical version, high-detail realistic style Chinese TikTok live screenshot, Elon Musk is talking to the mobile phone camera in the live broadcast room... There are obvious Chinese TikTok interface elements including likes, comments and share icons arranged vertically on the right, scrolling Chinese bullet screens... There is an eye-catching gift prompt special effect: 'Shinning sent TikTok No. 1'..."
> 要点：极详细的直播 UI 描述，包括弹幕、礼物特效、在线人数。适合作为"如何写出高质量直播 UI prompt"的 few-shot 示例。

**特朗普×金正恩抖音 PK 截图**
> Prompt: "生成特朗普和金正恩在抖音直播间打PK的截图"
> 要点：双人 PK 直播是抖音特色功能。需要左右分屏、PK 进度条、双方直播间 UI。

**VTuber 直播 UI 叠加**
> Prompt: "An anime-style illustration of a cute tanuki-girl VTuber streaming live... The image features a streaming UI overlay with 3 distinct components: a red badge reading 'LIVE', a pink badge reading 'ON AIR', and a semi-transparent dark grey chat box containing exactly 3 messages."
> 要点：动漫角色+直播 UI 叠加层。UI 元素要与动漫风格统一，不能用写实 UI 套在动漫角色上。
...
> - "抖音直播间的绝美女主播"
> - "美女举牌感谢大哥打赏大火箭"
> 要点：这类直播截图核心不是平台品牌，而是“强戏剧性话题 + 高情绪弹幕 + 夸张礼物反馈”。适合纳入本模板，而不是误归到社媒帖子详情页。

### 吸收规则（社交媒体直播子类）

- 有实时弹幕、礼物、在线人数、PK 条、购物车、主播话术 → 留在本模板
- 如果只是平台帖子 / 主页 / 评论区，没有直播时间感 → 改走 `social-interface-mockup.md`
- 如果只是直播预告封面而不是直播间内页 → 改走 `short-video-cover-ui.md`
- 如果是体育转播 / 新闻转播而非直播带货语法 → 不要硬套进本模板，应单独按转播画面语言处理

## Hybrid retrieval metadata（Phase 1 pilot）

- Associated template IDs: `ui-screenshot-system`
- Retrieval styles: `UI`
- Retrieval scenes: `Tech`, `Social`
- Retrieval tags: `UI`, `Dashboard`, `Screenshot`
- Curated example cases: `17`, `2`, `4`
- Crosswalk source: `data/template-crosswalk.json`
- Retrieval index entry: `data/retrieval-index.json#templates[id=ui-screenshot-system]`
