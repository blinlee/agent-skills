...
```text
基于 REFERENCE_0 中的人物（如无参考图，则按 {argument name="subject self description" default="东亚年轻女性，自然微笑"} 描述），将其转换为 {argument name="character" default="紫黑配色、长直发、带雷电感头饰的冷感女战士"} 风格的 cosplay 自拍照，
拍摄场景：{argument name="event location" default="上海漫展现场"}；
保留人物本人五官特征，让人能看出"是 ta 在 cos 这个角色风格"；
渲染为手机自拍照风格 + 现场氛围 + 自然光。
```

将上面的 `{argument name="character" ...}` 理解为**角色特征描述槽位**，优先写成服装 / 发型 / 配色 / 气质描述，而不是直接点名受版权保护角色。
...
- 尽量不要直接点名受版权保护角色；优先改写为服装 / 发型 / 配色 / 气质等特征描述
