#!/usr/bin/env python3
"""Bilibili video transcript extraction for anything2md.

This internal decoder adapts the tested single-video flow from the local
`bilibili-auto-transcript` reference: prefer human subtitles, then Bilibili AI
subtitles, then local Whisper transcription. It intentionally does not include
favorite-folder scanning, RAG indexing, notifications, or summary generation.
"""
...
def decode_bilibili_url(
    ...
    with TemporaryDirectory(prefix="anything2md-bilibili-") as tmp_dir:
        ...
        if chinese_sub_langs:
            transcript, subtitle_language = download_subtitle(
                source,
                cookies.args,
                tmp_root,
                chinese_sub_langs,
        ...
        if transcript:
            transcript_source = f"B站CC字幕 ({subtitle_language or 'unknown'})"
        else:
            if chinese_ai_langs:
                transcript, subtitle_language = download_subtitle(
                    source,
            ...
            if transcript:
                transcript_source = f"B站AI字幕 ({subtitle_language or 'unknown'})"
        if not transcript:
            if other_sub_langs:
                transcript, subtitle_language = download_subtitle(
                    source,
                ...
                if transcript:
                    transcript_source = f"B站CC字幕 ({subtitle_language or 'unknown'})"
            if not transcript and other_ai_langs:
                transcript, subtitle_language = download_subtitle(
                    source,
                    cookies.args,
                    tmp_root,
                ...
                if transcript:
                    transcript_source = f"B站AI字幕 ({subtitle_language or 'unknown'})"
            if transcript and not is_chinese_lang(subtitle_language):
